...
class SleepNumberPlatform {
    ...
    async addAccessories() {
        ...
        this.snapi.json.beds.forEach(
            async function(bed, index) {
                        ...
                        try {
                            ...
                            if (this.hasFoundation) {
                                if (!this.accessories.has(sideID + "flex")) {
                                    ...
                                    bedSideFlex.addService(Service.Lightbulb, sideName + "FlexHead", "head").addCharacteristic(Characteristic.Brightness);
                                    bedSideFlex.addService(Service.Lightbulb, sideName + "FlexFoot", "foot").addCharacteristic(Characteristic.Brightness);
    ...
    configureAccessory(accessory) {
        ...
        switch (accessory.context.type) {
            case "occupancy":
                ...
                var bedSideOccAccessory = new snOccupancy(this.log, accessory);
                bedSideOccAccessory.getServices();
                this.accessories.set(accessory.context.sideID, bedSideOccAccessory);
                break;
            case "number":
                accessory.reachable = true;
                var bedSideNumAccessory = new snNumber(this.log, accessory, this.snapi);
                bedSideNumAccessory.getServices();
                this.accessories.set(accessory.context.sideID, bedSideNumAccessory);
                break;
            case "flex":
                accessory.reachable = true;
                var bedSideFlexAccessory = new snFlex(this.log, accessory, this.snapi);
                bedSideFlexAccessory.getServices();
                this.accessories.set(accessory.context.sideID, bedSideFlexAccessory);
                break;
            case "privacy":
                accessory.reachable = true;
                var bedPrivacyAccessory = new snPrivacy(this.log, accessory, this.snapi);
                bedPrivacyAccessory.getServices();
...
class snOccupancy {
    ...
    getServices() {
        ...
        informationService.setCharacteristic(Characteristic.Manufacturer, "Sleep Number").setCharacteristic(Characteristic.Model, "SleepIQ").setCharacteristic(Characteristic.SerialNumber, "360");
...
class snNumber {
    ...
    getServices() {
        ...
        this.numberService.getCharacteristic(Characteristic.On).on(
            "change",
